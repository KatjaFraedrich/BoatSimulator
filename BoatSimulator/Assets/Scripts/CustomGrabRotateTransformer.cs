...
{
    public class CustomGrabRotateTransformer : MonoBehaviour, ITransformer
    {
        public enum Axis
        {
            Right = 0,
        ...
        public void UpdateTransform()
        {
            ...
            Vector3 eulerRotation = targetTransform.localEulerAngles;
            if (Constraints.MinAngle.Constrain)
            {
                eulerRotation = new Vector3(0, 0,Mathf.Max(eulerRotation.z, Constraints.MinAngle.Value));
            }
            if (Constraints.MaxAngle.Constrain)
            {
                eulerRotation = new Vector3(0, 0, Mathf.Min(eulerRotation.z, Constraints.MaxAngle.Value));
            }
            targetTransform.localEulerAngles = eulerRotation;
